...
class TaskSuggester:
    ...
    @staticmethod
    def suggest_from_commits(commits: List[Dict[str, str]]) -> List[str]:
    ...
    @staticmethod
    def suggest_from_issues(issues: List[Dict[str, str]]) -> List[str]:
    ...
    @staticmethod
    def suggest_from_pulls(pulls: List[Dict[str, str]]) -> List[str]:
    ...
    def aggregate_suggestions(
        ...
        suggestions.extend(TaskSuggester.suggest_from_commits(commits))
        suggestions.extend(TaskSuggester.suggest_from_issues(issues))
        suggestions.extend(TaskSuggester.suggest_from_pulls(pulls))

        if not suggestions:
            return ["Project is in a healthy state. Maintain current pace."]
