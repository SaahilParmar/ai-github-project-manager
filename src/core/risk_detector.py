...
class RiskDetector:
    ...
    def detect_commit_risks(commits: List[Dict[str, str]]) -> List[str]:
        ...
        if fix_count > len(commits) * 0.5:
            risks.append(
                "High number of bug fixes: potential instability."
            )

        if len(commits) < 3:
            risks.append(
                "Low commit activity: possible development slowdown."
            )
    ...
    def detect_issue_risks(issues: List[Dict[str, str]]) -> List[str]:
        ...
        if open_issues > len(issues) * 0.7:
            risks.append(
                "High number of open issues: backlog may be growing."
            )
    ...
    def detect_pr_risks(pulls: List[Dict[str, str]]) -> List[str]:
        ...
        if open_pulls > 5:
            risks.append(
                "Too many open pull requests: review bottleneck."
            )
